...
// ignoredDirNames are common build/dependency-artifact directory names skipped the same way
// dot-directories already are - a real, working example built the normal way for its ecosystem
// (an install step, a build/test run) regenerates these, and they are never part of the pattern
// being learned. Not exhaustive by design: a fixed list covering the common cases, not a
// configurable ignore mechanism (issue #54).
var ignoredDirNames = map[string]bool{
	"node_modules": true, // npm/yarn/pnpm
	"dist":         true, // common JS/frontend build output
	"build":        true, // general build output (also Python setuptools)
	"target":       true, // Rust cargo, Java/Maven
	"bin":          true, // .NET build output
	"obj":          true, // .NET build intermediates
...
// did and didn't leave the machine). Dot-directories (.git and similar), common build/dependency-
// artifact directories (node_modules and similar - see ignoredDirNames), and anything that sniffs
...
func Scan(dir string) ([]SourceFile, []string, error) {
	...
	err = filepath.WalkDir(dir, func(path string, d os.DirEntry, err error) error {
		...
		if d.IsDir() {
			...
			// A build/dependency-artifact directory, regenerated by the ecosystem's own tooling
			// rather than authored - see ignoredDirNames.
